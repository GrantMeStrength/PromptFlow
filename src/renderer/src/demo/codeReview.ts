  ...
  reviewedAt: new Date().toISOString(),
    ...
    },
    {
      ...
      data: {
        ...
        code: '',
        prompt: '',
        noteText: 'The "Review Focus" field shapes what the LLM pays attention to — try "security vulnerabilities", "performance", "readability", or "test coverage" to get a completely different review. The Parse Code Stats function node runs independently of the LLM: it counts lines, functions, classes, and imports using regex patterns directly on the source text, so those stats appear instantly without an API call. Both branches converge at the Format Review node, which combines the LLM commentary with the computed stats into a final report.',
      },
    },
  ],
  edges: [
