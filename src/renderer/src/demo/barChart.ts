...
return { __html: html }`,
    ...
    },
    {
      ...
      data: {
        ...
        code: '',
        prompt: '',
        noteText: 'Two text inputs feed a single LLM node — "Chart Topic" sets the title, while "Categories to Chart" tells the model exactly which items to assign percentages to. The LLM returns a JSON array of { label, value } objects, which the Function node parses, sorts by value, and renders as coloured SVG bars using inline HTML — no external charting library required. To customise: change the categories input, or modify the colour logic in the Render Bar Chart node.',
      },
    },
  ],
  edges: [
