...
return { __html: html }`,
    ...
    },
    {
      ...
      data: {
        ...
        code: '',
        prompt: '',
        noteText: 'Two inputs feed a single LLM node — the topic sets the chart title while the options list tells the model what to generate percentages for. The function node then sorts and renders the bars using inline HTML, so no external charting library is needed.',
      },
    },
  ],
  edges: [
