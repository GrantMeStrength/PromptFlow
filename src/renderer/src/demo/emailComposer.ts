  ...
  nodes: [
    {
      ...
      data: {
        ...
        code: '',
        prompt: '',
        noteText: 'Enter a recipient name, topic, and tone, then press Run. An LLM drafts a polished email, which is then passed through a metadata node that appends word count, reading time, and a timestamp before final output. A state node tracks how many drafts you have composed — each run increments the draft counter so you can see it accumulate.',
      },
    },
    {
      id: 'ui-tone',
    ...
    },
    {
      ...
      data: {
        label: 'Draft Count',
        kind: 'state',
        description: 'Reads the number of email drafts composed so far in this session.',
        inputs: [],
        outputs: [{ name: 'value', type: 'number', description: 'Drafts composed so far' }],
        stateKey: 'emailDraftCount',
        stateDefault: '0',
        stateMode: 'read',
        code: '',
        prompt: '',
...
return { email }`,
    ...
    },
    {
      ...
      data: {
        ...
        kind: 'function',
        description:
          'Attaches word count, reading time, a draft number (incremented from state), and a timestamp to the drafted email.',
        inputs: [
          { name: 'email', type: 'string', description: 'Drafted email body' },
          { name: 'draftCount', type: 'number', description: 'Previous draft count from state' },
        ],
        outputs: [
          ...
          { name: 'draftNumber', type: 'number', description: 'This draft\'s number (1-based, persists across runs)' },
          { name: 'generatedAt', type: 'string', description: 'ISO timestamp' },
        ],
        code: `const email = String(inputs.email ?? '')
const draftNumber = Number(inputs.draftCount ?? 0) + 1
...
return {
  ...
  generatedAt: new Date().toISOString(),
        ...
        prompt: 'Add word count, reading time, draft number, and timestamp metadata to the email',
      },
    },
    {
      id: 'state-write-count',
      type: 'state',
      position: { x: 980, y: 80 },
      data: {
        label: 'Save Draft Count',
        kind: 'state',
        description: 'Persists the updated draft count so the next run knows how many drafts have been composed.',
        inputs: [{ name: 'value', type: 'number', description: 'New draft count to store' }],
    ...
    },
    {
      ...
      data: {
        ...
        kind: 'output',
        description:
          'Final output node — returns the email text along with its metadata and draft number.',
        inputs: [
          { name: 'email', type: 'string', description: 'Email body' },
          { name: 'wordCount', type: 'number', description: 'Word count' },
          { name: 'readingTimeSec', type: 'number', description: 'Reading time' },
          { name: 'draftNumber', type: 'number', description: 'Draft number' },
          { name: 'generatedAt', type: 'string', description: 'Timestamp' },
        ],
        outputs: [],
        code: `return {
  draft: \`Draft #\${inputs.draftNumber}\`,
  email: inputs.email,
  wordCount: inputs.wordCount,
  readingTimeSec: inputs.readingTimeSec,
  generatedAt: inputs.generatedAt,
}`,
        prompt: 'Output the finished email with its metadata and draft number',
      },
    },
    {
      id: 'note-metadata',
  ...
  edges: [
    ...
    {
      id: 'e-count-meta',
      source: 'state-read-count',
      target: 'fn-meta',
      sourceHandle: 'value',
      targetHandle: 'draftCount',
      animated: true,
    },
    {
      id: 'e-meta-save-count',
      source: 'fn-meta',
      target: 'state-write-count',
      sourceHandle: 'draftNumber',
      targetHandle: 'value',
      animated: true,
    },
    {
